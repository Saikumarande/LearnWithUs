...
(async()=>{
  try{
    ...
    for(const [url,status,type] of checks){const response=await fetch(`http://127.0.0.1:${port}${url}`);if(response.status!==status)throw new Error(`${url}: expected ${status}, received ${response.status}`);checksPassed++;if(!response.headers.get('content-type')?.includes(type))throw new Error(`${url}: expected ${type}, received ${response.headers.get('content-type')}`);checksPassed++;}
    console.log(`Server route checks passed: ${checksPassed}/${checks.length*2} assertions across ${checks.length} URLs, MIME types and custom 404.`);
