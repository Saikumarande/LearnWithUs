...
(() => {
  ...
  function batches(){
    const all=items();
    if(mode==='words')return [all.slice(0,10),all.slice(10)].filter(batch=>batch.length);
    if(mode==='numbers')return Array.from({length:Math.ceil(all.length/20)},(_,i)=>all.slice(i*20,i*20+20));
    return [all];
  }
  function pageForIndex(index){return mode==='numbers'?Math.floor(index/20):mode==='words'&&index>=10?1:0;}
  ...
  function render(){
    ...
    ui.cardCount.textContent=mode==='letters'?'All 26 letters':mode==='words'?'A–J, then K–Z':mode==='animals'?animals.length+' animals together':'20 numbers per page';
    ...
    ui.nextPage.textContent=mode==='words'?'Next: K–Z →':mode==='numbers'&&page<pages.length-1?'Next '+pages[page+1].length+' numbers →':'Next →';
