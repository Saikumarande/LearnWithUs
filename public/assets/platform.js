...
(() => {
  ...
  function offlineStatus(){
    const top=document.querySelector('.fl-topbar');if(!top||document.getElementById('lw-online-status'))return;
    const el=document.createElement('span');el.id='lw-online-status';el.className='lw-online-status';el.setAttribute('role','status');top.append(el);
    const update=()=>{el.textContent=navigator.onLine?'Online':'Offline · saved learning still works';el.classList.toggle('is-offline',!navigator.onLine);};
  ...
  function injectHeader(){
    const menu=document.getElementById('fl-menu');if(!menu)return;
  ...
  applyAccessibility();injectHeader();accessibilityPanel();offlineStatus();injectBookmark();injectBreadcrumb();searchModal();recordVisit();homeHub();dashboard();activityHooks();consent();pwa();learningTimer();
})();
