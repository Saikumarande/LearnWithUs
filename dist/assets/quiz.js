'use strict';
(() => {
function saveQuiz(title,score,total,url,mistakes=[]){
 if(window.LearnWithUs?.recordQuiz){window.LearnWithUs.recordQuiz(title,score,total,url,mistakes);return;}
 try{const key='learnwithus.platform.v1',state=JSON.parse(localStorage.getItem(key)||'{}')||{};state.quizHistory=Array.isArray(state.quizHistory)?state.quizHistory:[];state.completed=Array.isArray(state.completed)?state.completed:[];state.mistakes=Array.isArray(state.mistakes)?state.mistakes:[];state.quizHistory.unshift({title,score,total,url,time:new Date().toISOString()});state.quizHistory=state.quizHistory.slice(0,30);if(!state.completed.includes(url))state.completed.push(url);mistakes.forEach(tag=>{if(tag)state.mistakes.push({tag,title,url,count:1,time:new Date().toISOString()});});state.mistakes=state.mistakes.slice(-40);localStorage.setItem(key,JSON.stringify(state));}catch{}
}
const sets={
easy:[
...
function showResult(){
 ...
 saveQuiz(names[level]+' food quiz',score,current.length,'quiz.html?level='+level,missed);
