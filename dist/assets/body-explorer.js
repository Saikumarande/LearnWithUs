...
(() => {
  ...
  function videoPanel(part){
    if(part.video){const v=part.video;return `<section class="body-video" aria-labelledby="video-title"><p class="eyebrow">See it in motion</p><h3 id="video-title">${escape(v.title)}</h3><p>${escape(v.description)} This is an explanatory animation.</p><div class="body-video-actions"><button class="primary-button" id="load-body-video" type="button" aria-controls="body-player" aria-expanded="false">▶ Open animation</button><button class="text-button" id="close-body-video" type="button" hidden>Close video</button></div><div class="body-player" id="body-player" hidden></div><p class="body-video-credit">The player loads from YouTube only after you choose to open it. Medical Animation Copyright © Nucleus Medical Media. All rights reserved.</p></section>`;}
    if(part.id==='kidneys')return `<section class="body-video" id="kidney-video" tabindex="-1"><p class="eyebrow">Prefer to watch?</p><h3>See what the kidneys do</h3><p>Learn how the kidneys filter blood, balance water and salts, and help remove wastes.</p></section>`;
    return '';
  }
  function render(focus=false){
    ...
    detail.innerHTML=`<div class="body-toolbar"><button type="button" class="text-button" id="all-body-parts">← All body parts</button><label for="body-select">Explore another part<select id="body-select">${parts.map(item=>`<option value="${item.id}"${p.id===item.id?' selected':''}>${escape(item.name)}</option>`).join('')}</select></label></div>
      <div class="body-overview"><div class="body-summary"><p class="eyebrow">${escape(p.category)} · About 3 min</p><h2 id="body-title" tabindex="-1">Your ${p.name.toLowerCase()}.</h2><p class="body-tagline">${escape(p.tagline)}</p><p>${escape(p.intro)}</p></div><figure class="body-image"><img src="${p.image}" alt="${escape(p.alt)}" width="600" height="450"><figcaption>${escape(p.imageCredit)}.</figcaption></figure></div>
